...
class Detector:
	...
	def __init__(self, image, scale_factor, minimum_neighbors, minimum_size, flags, classifier,cap):
	...
	def detect(self,image,cap):
		...
		for (x, y, w, h) in detected_faces:
			#face = Face(self.image, x, y, w, h, 0)
			crop = self.image[y:y+w,x:x+h]
			cv2.imshow('FACE TRACKING',crop)
			im = im.resize((w, h))
			temp = self.image[:,:]
			self.image[y:y+w,x:x+h] = im
			self.image[:,:] = temp
			#face.draw(2)
